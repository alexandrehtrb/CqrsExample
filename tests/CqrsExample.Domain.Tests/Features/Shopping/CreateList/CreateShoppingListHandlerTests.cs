...
namespace CqrsExample.Domain.Tests.Feature.Shopping.CreateList
{
    public class CreateShoppingListHandlerTests
    {
        ...
        private static readonly CreateShoppingListCommand invalidCmd = new CreateShoppingListCommand();
        private static readonly CreateShoppingListCommand validCmd = new CreateShoppingListCommand() { Title = testTitle };
        ...
        public CreateShoppingListHandlerTests()
        {
            ...
            handler = new CreateShoppingListHandler(repository.Object);
        }
        ...
        {
            // GIVEN
            repository.Setup(r => r.InsertAsync(It.Is<ShoppingList>(s => s.Title == testTitle)))
                      .ReturnsAsync(false);
            ...
            // THEN
            repository.Verify(r => r.InsertAsync(It.Is<ShoppingList>(s => s.Title == testTitle)), Times.Once);
            Assert.False(result.IsSuccess);
